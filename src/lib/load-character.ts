...
function hydrate(row: CharacterRow): HydratedCharacter {
  ...
  return {
    ...
    // max = override MJ si défini, sinon dérivé de la formule du jeu
    maxHp: row.maxHpOverride ?? BASE_HP + bonus + constiScore,
    maxMental: row.maxMentalOverride ?? BASE_MHP + bonus + psyScore,
    maxEndurance: row.maxEnduranceOverride ?? enduTier.max,
    maxHpOverride: row.maxHpOverride ?? null,
    maxMentalOverride: row.maxMentalOverride ?? null,
    maxEnduranceOverride: row.maxEnduranceOverride ?? null,
    maxFluxOverride: row.maxFluxOverride ?? null,
    ...
    maxFlux: row.maxFluxOverride ?? fluxTier.max,
